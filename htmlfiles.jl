...
# ╔═╡ 6dedb5be-47a7-11ef-0506-e356ab0e75cb
begin
	using PlutoUI
	using CitableBase
	using CitableText
...
# ╔═╡ b1df4c18-4ddb-4ea8-bbb3-def1519e66b8
using HmtArchive, HmtArchive.Analysis

# ╔═╡ 08cd179c-aad3-42bb-b316-371142589c91
using Printf

# ╔═╡ 22cc1883-be2d-4439-a033-2304e10ea7cd
src = hmt_cex()

# ╔═╡ c38e2cce-9b60-4019-bad9-c376d7537629
textcatalog = hmt_textcatalog(src)

# ╔═╡ 99971065-4bec-4319-9905-4afe7fa7af65
diplomatictexts = hmt_diplomatic(src)

# ╔═╡ b2de29f2-abe6-4271-88ca-d2688c52f8cd
normalizedtexts = hmt_normalized(src)

# ╔═╡ 9cadccf6-05d0-423c-b11f-90a7c13af60b
md"""# Making HTML Files"""

# ╔═╡ 3fd7c50b-5bc0-4ec6-b776-8ba973b281a6
md"""Filter out what we don't want, use a sliding window to split book 1 into sections, and create HTML files."""

# ╔═╡ ba588269-5a9a-40d8-8a9f-1bb9fc2e9180
diplomatictexts.passages

# ╔═╡ 68e53aa2-f7ef-4764-8213-c0b2d0f5cdb2
# Just MsA URNs
begin
	justMsAUrns = filter(diplomatictexts.passages) do p
		msAUrns = CtsUrn("urn:cts:greekLit:tlg0012.tlg001.msA:")
		urncontains(msAUrns, p.urn)
	end
end

# ╔═╡ 10871213-9cb3-4dd5-a94b-ea1ad3f92530
# Just URNs
begin
	# Maps do something to everything
	justURNs = map(justMsAUrns) do p
		p.urn
	end

end

# ╔═╡ fb8e2d1d-1973-432b-890f-798f4c2656c1
# Drop Exemplar
begin
	versionURNs = map(justURNs) do u
		tempU = dropexemplar(u)
		noPassageU = collapsePassageTo(tempU, 1)
	end
end

# ╔═╡ 5c6cc612-1b34-46a2-9f83-a3ca338aee86
iliadBookURNs = unique(versionURNs)

# ╔═╡ c5bf76c2-7934-45ca-b53b-f3e3888fe234
iliadBookURNs[22:end]

# ╔═╡ 4dd6c5d0-53e5-4afc-a727-91f3bea20a7e
md"""### Get just 1 Book"""

# ╔═╡ bf5aed1c-af97-4959-8cd5-38cc7b207217
md"""### Make a sliding window to split the book further"""

# ╔═╡ 5c8646ed-ee83-466e-94c4-3bc82162f004
function sliding(arr, size, step=1; include_leftover=true)
    n = length(arr)
    windows = []
    i = 1
    while i <= n - size + 1
        push!(windows, arr[i:i+size-1])
        i += step
    end
    if include_leftover && i <= n
        push!(windows, arr[i:end])
    end
    return windows
end

# ╔═╡ 4b61b717-b293-48e1-a125-be9b89a6d69c
md"""## Assemble the HTML file"""

# ╔═╡ 067886b4-e4dd-4fe9-a365-6956d7de0fb3
function save_string_to_file(content::String, filepath::String)
...
# ╔═╡ 86ea0213-42ae-46dc-8a30-33bb25fe7c62
htmlhead = """
...
# ╔═╡ 6102a962-8a32-4f07-8c3b-8bdbb79ece92
htmlfoot = """
...
# ╔═╡ 4be609ae-ee66-46de-a026-ee0147a5f44e
filepath = "/Users/alysoncramer/Desktop/book_one.html"

# ╔═╡ a57f97de-77f2-4831-9691-7c95b76e6ce6
# ╠═╡ disabled = true
#=╠═╡
save_string_to_file(myhtml, filepath)
  ╠═╡ =#

# ╔═╡ ec5b5ab0-f209-407d-a12d-cb4bf2a1b3b8
function save_split_book(strings::Vector{Any}, directory::String)
    ...
    for i in 1:length(strings)
        filename = @sprintf("%sbook_1_%03d.html", directory, i)
        open(filename, "w") do file
            println(file, string(htmlhead, join(strings[i], "\n"), htmlfoot))
        end
    end
end

# ╔═╡ bc89cf40-b490-4ade-b879-8b047ecd99b9
directory = "/Users/alysoncramer/Desktop/The_Iliad/Book 1"

# ╔═╡ d516c8cc-c71a-439f-9023-715cd73cf914
begin
	myBookPassage = filter(diplomatictexts.passages[2:end]) do p
		myUrn = iliadBookURNs[1]
		myPassage = urncontains(myUrn, p.urn)
	end
end

# ╔═╡ 274badb0-03ec-4b22-b3e7-0039c328a5e0
begin
	prettyPrintBook = map(myBookPassage) do p
		justPassage = passagecomponent(p.urn)
		thisText = p.text
		"""<p>$justPassage $thisText</p>"""
	end
end

# ╔═╡ 3a358587-8be4-4d45-9efc-98f3c498609c
bookOne = join(prettyPrintBook, "\n")

# ╔═╡ 342bddf6-e4fa-4ee6-a953-c2daed698bf9
htmlbody = """
...
# ╔═╡ c89c84cb-a15b-4e3a-a75f-d22fc0916a47
myhtml = string(htmlhead, htmlbody, htmlfoot)

# ╔═╡ fcb32f0b-2622-4c22-a968-e2d6ed04328c
splitBook = sliding(prettyPrintBook, 50, 50)

# ╔═╡ ad7236fd-e4ec-4a33-afd6-708aa7dd8d1f
# ╠═╡ disabled = true
#=╠═╡
save_split_book(splitBook, directory)
  ╠═╡ =#

# ╔═╡ 01746e65-85d3-428d-aa1b-c62f490a34e0
md""" 
Make it go through the whole *Iliad*. Make the file names have the line numbers "1-49". 

- differences between diplomatic and normalized.
- how do we show scolia? Maybe a footnote with links to jump to the bottom then back to the top.
"""

# ╔═╡ 7d57541f-024b-4ba9-b100-73e2927c9ec5
md"""# The *Iliad* but with Footnotes"""

# ╔═╡ fd3b188f-3200-4ca4-9e48-031b228d8ec4
begin
	prettyPrintBookLink = map(myBookPassage) do p
		justPassage = passagecomponent(p.urn)
		thisText = p.text
		"""$justPassage $thisText"""
	end
end

# ╔═╡ 1eb85633-881e-4046-9aad-7e2d0c3b313e
linesLink = sliding(prettyPrintBookLink, 25, 25)

# ╔═╡ 74c21c1d-2766-45dd-9741-cbe0e5a2eac2
function generate_html_with_anchors(lines)
    html_content = "<body>\n"

    for (i, line) in enumerate(lines)
        anchor_name = "line$i"
        html_content *= "<p id=\"$anchor_name\">$line</p>\n"
    end
...
# ╔═╡ f94b4401-e651-48e7-8dc6-b67d1d711271
linkedLines = generate_html_with_anchors(linesLink[1])

# ╔═╡ 40f28d45-f464-482e-bed4-1dbff74c6185
htmlheadLink = """
...
# ╔═╡ be54a2e4-8c0c-4b6e-85ef-7bb923eae396
htmlbodyLink = """

<h2 id="top">ἰλιάδος ἄλφα</h2>
...
# ╔═╡ dbc68848-f9e4-4c58-bd9d-ba801ecca3ba
htmlfootLink = """
...
# ╔═╡ e2fb23a6-83bc-4677-b32b-e1c23632d28a
filepathLinked = "/Users/alysoncramer/Desktop/iliadLinked.html"

# ╔═╡ 5815a893-50ee-4959-82a7-ae4a1f7a28a3
myhtmlLinked = string(htmlheadLink, htmlbodyLink, htmlfootLink)

# ╔═╡ 558ecd52-9efa-48b0-b9f3-131b97377264
# ╠═╡ disabled = true
#=╠═╡
save_string_to_file(myhtmlLinked, filepathLinked)
  ╠═╡ =#

# ╔═╡ 93fce903-161a-4300-aa9c-a9609433b33c
md"""### Now for the scholia"""

# ╔═╡ 8ee8f58c-d6d2-4a7b-8ac2-3bff8999f60f
urns = CtsUrn("urn:cts:greekLit:tlg0012.tlg001.msA:1.1")

# ╔═╡ c94c5807-1dc5-4e39-8ae1-e70f7412e9cb
diplomatictexts.passages[25].urn

# ╔═╡ a62bd88e-ac47-4896-9c37-1506041ef63b
scholia_index = hmt_commentary(src)[1]

# ╔═╡ 00354b7c-551e-4fe8-ba5f-4f0995da88a5
scholia_text_md_on(urns, normalizedtexts, scholia_index)

# ╔═╡ 0a2bddff-5b94-4f0e-af9d-f76a568e8244
scholion(urns, normalizedtexts)

# ╔═╡ 00000000-0000-0000-0000-000000000001
PLUTO_PROJECT_TOML_CONTENTS = """
[deps]
CitableAnnotations = "63f90415-d3ea-41a7-8107-8cc186a9b888"
...
manifest_format = "2.0"
project_hash = "08d7343ec650d8472d96daa377479ca2bef6623c"
...
[[deps.HmtArchive]]
deps = ["CitableAnnotations", "CitableBase", "CitableCollection", "CitableCorpus", "CitableImage", "CitableObject", "CitableParserBuilder", "CitablePhysicalText", "CitableTeiReaders", "CitableText", "CiteEXchange", "Compat", "DocStringExtensions", "Documenter", "Downloads", "EditionBuilders", "EditorsRepo", "EzXML", "FileIO", "FreqTables", "HTTP", "Pkg", "Plots", "PolytonicGreek", "Query", "SplitApplyCombine", "StatsBase", "Tables", "Test", "TestSetExtensions", "TypedTables", "ZipFile"]
git-tree-sha1 = "403a46132d6e95c549ff59e729400d35731c9c28"
